...
export default class BasePage {
  ...
  revoke() {
    ...
    if (this.isMounting)
      this.instanceCallback.onMount = async () => {
        await this.unmount();
        this.revokeEvent = false;
        this.instanceCallback.onMount = null;
      };
    else this.unmount().then(() => (this.revokeEvent = false));
  }
  ...
  async mount(
    ...
    if (this.instanceCallback.onPreload) {
      await this.instanceCallback.onPreload();
    }
    this.pageRoot = await this.mountContent();
    if (callbacks.onContentReady) {
      await callbacks.onContentReady();
    }
    if (this.instanceCallback.onContentReady) {
      await this.instanceCallback.onContentReady();
    }
    ...
    if (this.instanceCallback.onMount) {
      await this.instanceCallback.onMount();
    }
  ...
  async unmount() {
    ...
    if (this.instanceCallback.onUnmount) {
      await this.instanceCallback.onUnmount();
    }
